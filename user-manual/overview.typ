...
The Hardware Build System (HBS) is a build system for hardware design projects.
...
HBS tries to support all file types that might potentially be required to generate a final result.
HBS is not limited to managing only pure hardware description files.

The HBS can be described as a Tcl-based, minimal common abstraction approach for build system for hardware designs.
...
The HBS is designed in such a way that it is straightforward to utilize EDA tools' exclusive features.
...
The HBS is not a tool for preparing Tcl code that is only later executed by EDA tools.
The HBS is a tool which code is executed directly by EDA tools.
